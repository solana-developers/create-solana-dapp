...
describe("create-solana-dapp", async () => {
    ...
    before(() => {
        ...
        console.log();
        execSync(`${cliCommand}${testDappNameNextAnchor} -f next -p anchor`, { stdio: "ignore" });
    ...
    it("Invalid UI framework", async () => {
        ...
        assert(!(await testCli(cliCommand + testDappNameNotCreated, invalidUiFrameworkOpt)));
    ...
    it("Invalid program framework", async () => {
        ...
        assert(!(await testCli(cliCommand + testDappNameNotCreated, invalidProgramFrameworkOpt)));
        let dirCreated: boolean = fs.existsSync(testDappNameNotCreated);
        if (dirCreated) {
            execSync(`rm -rf ${testDappNameNotCreated}`, { stdio: "ignore" });
        };
    ...
    async function testYarnInstall(dappName: string, subdir: string): Promise<void> {
    ...
    it("The program builds successfully for native", async () => {
        await programBuilds("cargo build-bpf --manifest-path=./program/Cargo.toml --bpf-out-dir=./program/target/solana", testDappNameNextNative);
    });

    async function programRuns(commands: string[], dappName: string): Promise<void> { 
        process.chdir(path.join(originalDir, dappName, "program"));
    ...
    async function swapAnchorProgramId(dappName: string) {
        process.chdir(path.join(originalDir, dappName, "program"));
        const anchorDefaultProgramId: string = "Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS";
        const programId: string = JSON.parse(fs.readFileSync(
            path.join(process.cwd(), "target", "idl", `${dappName.replace(/-/g, '_')}.json`), "utf-8"
        )).metadata.address;
        const filesToChange = [
            `Anchor.toml`,
        ...
        for (var file of filesToChange) {
            ...
            fs.readFile(pathToFile, 'utf-8', (err, data) => {
                ...
                fs.writeFile(pathToFile, data.replace(anchorDefaultProgramId, programId), 'utf-8', (err) => {
                    if (err) return console.log(err);
                });
            });
    ...
    it("The program test runs successfully for native", async () => {
        await programRuns([
            ...
            `solana program deploy ./program/target/solana/${testDappNameNextNative.replace(/-/g, '_')}.so`,
            "yarn run test",
    ...
    after( async () => {
        ...
        console.log();
        await new Promise( resolve => setTimeout(resolve, 10*1000) );
        execSync(`rm -rf ${path.join(process.cwd(), testDappNameNextAnchor)}`, { stdio: "ignore" });
