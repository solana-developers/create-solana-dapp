...
function printDirectoryInstructions() {
  console.log();
  console.log(
    `${chalk.redBright("Error:")} Please specify the project directory:`,
  );
  console.log();
  console.log(
    `  create-solana-dapp ${chalk.greenBright("<project-directory>")}`,
  );
  console.log();
  console.log();
...
const program: Commander.Command = new Commander.Command(packageJson.name)
  ...
  .usage(
    `${chalk.green("<project-directory>")} ${chalk.yellowBright("[options]")}`,
  )
  .action(function (name: string) {
    if (
      ...
      console.error(
        `Invalid argument format for ${chalk.green("<project-directory>")}:`,
      );
  ...
  .option(
    ...
    `
  UI framework:                     [ Next | Vue ]          ${chalk.magentaBright(
    "Default:",
  )} [ Next ]
`,
  )
  .option(
    ...
    `
  Program development framework:    [ Anchor | Native ]     ${chalk.magentaBright(
    "Default:",
  )} [ Anchor ] 
`,
  )
...
async function run(): Promise<void> {
  ...
  if (!valid) {
    console.error(
      ...
      )} ${projectName} ${chalk.redBright(
        " : ",
      )} violates npm naming restrictions.`,
    );
    if (problems) {
  ...
  await createSolanaDapp({
    ...
    framework:
      ...
      undefined,
    program:
      (typeof options.program === "string" && options.program.trim()) ||
      undefined,
  });
}

run().catch(async function (reason) {
  {
    console.log();
