...
import { downloadFiles, validateFramework } from "./helpers/framework";
import { shouldUseGit, tryGitInit } from "./helpers/git";
import { shouldUseSolana } from "./helpers/solana";
import { renderProgramTemplates } from "./helpers/template";
import {
  installDeps,
  shouldUseYarn,
...
export async function createSolanaDapp({
  ...
  console.log(
    `${chalk.magentaBright("    UI Framework      : ")} ${framework}`,
  );
  ...
  console.log();
  console.log(
    `Installing dependencies for ${chalk.magentaBright(
      framework,
    )} UI framework...`,
  );
  console.log();
  ...
  console.log();
  console.log(
    `Successfully created Solana dApp: ${chalk.greenBright(dappName)}!`,
  );
  console.log();
